<!DOCTYPE html>
<html>
<head>

    <link rel="stylesheet" type="text/css" href="CSS\style.css">

</head>
<body>
@extends('layouts.header')



<div class="full_page">
    <div class="header_1">
        <div class="d3">
            <form>
                <input type="text" placeholder="Искать здесь...">
                <button type="submit"><img src="media\search.svg" class="search_main" type="submit"></button>
            </form>
        </div>
        <div class="header_icons">
            <img src="https://image.flaticon.com/icons/svg/2097/2097743.svg" width="36px " height="34px">

        </div>
    </div>
    <div class="stories">
        <b>Friends</b>
    </div>
    <div class="line_friends"></div>

    <div class="news_nadp"></div>

    <div class="friend_list">

        <div class="Friends">
            <img src="media\danil.jpg" class="photo_friend" height="30px " width="30px">
            <div class="info_friend"><b>Лихачев Данил<b></div>

            <div class="write_message"><u>write message<u></div>
        </div>


        <div class="line_friends"></div>


        <div class="Friends">
            <img src="media\danil.jpg" class="photo_friend" height="30px " width="30px">
            <div class="info_friend"><b>Лихачев Данил<b></div>
            <div class="write_message"><u>write message<u></div>
        </div>


        <div class="line_friends"></div>

        <div class="Friends">
            <img src="media\danil.jpg" class="photo_friend" height="30px " width="30px">
            <div class="info_friend"><b>Лихачев Данил<b></div>
            <div class="write_message"><u>write message<u></div>
        </div>


        <div class="line_friends"></div>


        <div class="Friends">
            <img src="media\danil.jpg" class="photo_friend" height="30px " width="30px">
            <div class="info_friend"><b>Лихачев Данил<b></div>

            <div class="write_message"><u>write message<u></div>
        </div>


        <div class="line_friends"></div>

        <div class="Friends">
            <img src="media\danil.jpg" class="photo_friend" height="30px " width="30px">
            <div class="info_friend"><b>Лихачев Данил<b></div>

            <div class="write_message"><u>write message<u></div>
        </div>


        <div class="line_friends"></div>















        <div class="right_bar">
            <li class="text_bar">Мои друзья:<div class="text"> 1</div></li>
            <li class="text_bar">запросы в друзья: <div class="text">223</div></li>
            <li class="text_bar">найти друзей:<div class="text"> 334</div></li>
        </div>



    </div>



</div>

</div>

</body>
</html>
<!--  -->
